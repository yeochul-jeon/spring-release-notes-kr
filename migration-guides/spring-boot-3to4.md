# Spring Boot 3.x → 4.x 마이그레이션 가이드
...
1. **Boot 3.x 최신**: 3.x에서 deprecated 경고가 4.0 마이그레이션의 핵심 가이드입니다.
2. **Deprecated 해결**: `@MockBean` → `@MockitoBean`, `ListenableFuture` → `CompletableFuture` 등을 처리합니다.
3. **Undertow 전환**: Tomcat 11(기본값) 또는 Jetty 12.1로 전환합니다.
4. **OpenRewrite**: `UpgradeSpringBoot_4_0` 레시피로 Jackson 3.x, 스타터 변경, 프로퍼티 등을 자동 변환합니다.
5. **프로퍼티 변경**: `server.error.*` → `spring.web.error.*`, `spring.jackson.*` 등을 수정합니다.
6. **테스트 수정**: `@MockitoBean` 전환, `@AutoConfigureMockMvc` 명시 추가 등을 적용합니다.
7. **Boot 4.0 의존성**: `build.gradle`/`pom.xml`에서 버전을 4.0으로 변경합니다.
8. **테스트 실행**: 전체 빌드 및 테스트로 나머지 문제를 해결합니다.
...
A: `org.openrewrite.java.spring.boot4.UpgradeSpringBoot_4_0` 복합 레시피로 Jackson, 스타터, 프로퍼티, deprecated API 등을 일괄 변환할 수 있습니다.
