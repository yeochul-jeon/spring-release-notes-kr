...
| **Prometheus Pushgateway** | `simpleclient_pushgateway` 대신 `prometheus-metrics-exporter-pushgateway` 사용, `base-url` 대신 `address` |
...
- **OpenTelemetry**: `OTEL_RESOURCE_ATTRIBUTES`, `OTEL_SERVICE_NAME` 환경 변수 지원
...
| `HttpOption.ENABLE_REDIRECTS` | RestTemplate 리다이렉트 설정 사용 |
